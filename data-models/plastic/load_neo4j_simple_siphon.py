...
def main():
    print("=" * 50)
    print("CIRCULOOS Neo4j Simple Siphon Example Loader")
    print("=" * 50)
    print()
    ...
    with Neo4jConnection() as conn:
        ...
        # Load data
        print("Loading siphon component data into Neo4j...")
        cypher_file = Path(__file__).parent / "neo4j-simple-siphon.cypher"
        ...
        for rel in rels:
            print(f"  {rel['RelationType']}: {rel['Count']}")
    
    print()
    print("=" * 50)
    print("✓ Simple siphon example loaded successfully!")
    print("=" * 50)
    print()
    ...
    print("  1. View all: MATCH (n) RETURN n")
    print("  2. View flow: MATCH path = (s:Company)-[*]-(c:ManufacturingComponent) RETURN path")
    print("  3. Count nodes: MATCH (n) RETURN labels(n)[0] as Type, count(n) as Count")
